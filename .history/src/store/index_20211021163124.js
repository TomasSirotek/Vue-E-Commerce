...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setMenuItems: state => {
      ...
      async getMetadata({ state}){
        const dataBase = await db.collection("menuItems").orderBy("data");
        const results = await dataBase.get();
        results.forEach((doc) => {
            if (!state.testArray.some((item) => item.gameId === doc.id)){
              const data = {
                gameID : doc.id,
                title : doc.data().title
              };
              state.testArray.push(data)
              console.log("mrdko => ",state.testArray)
            }
        })
      },
