...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async getAllUsers({ state }) {
      const dataBaseUsers = await db.collection("users").doc(firebase.auth().currentUser.uid);
      const dbResults = await dataBaseUsers.get();
      dbResults.forEach((doc) => {
        if (!state.collectionOfUsers.some(users => users.userID === doc.id)) {
          const data = {
            userId: doc.data(),

          };
          state.collectionOfUsers.push(data);
         
          console.log(" => action", state.collectionOfUsers)
