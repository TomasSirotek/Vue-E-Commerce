...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    removeItem(state, currentGame) {
      console.log("teacking delete basket", currentGame.gameID)
      

      
      let itemIcon = state.cart.find(i => i === currentGame)
      let indexItem = cart.indexOf(currentGame)

      
      if(itemIcon) {
        state.cart.splice(itemIcon, 1) 
              
        console.log("teacking delete item inside if", itemIcon)
      }
      else {}
  ...
  getters: {
    finalFilter(state){
       return state.games.filter((game) =>{
        return game.title.match(state.search)
      });
