...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    undoArchive(state, id) {
      let archiveItems = state.orderItems.filter(item => item.id === id)[0];

      if (archiveItems.archive === true && archiveItems.storeOrders === true) {
        dbOrders.doc(id).update({ archive: false, storeOrders: false })
  ...
  getters: {
    ...
    archiveItems(state) {
      return state.orderItems.filter(i => i.archive === true)
    ...
    totalPriceArchived: state => {
      let total = 0
      state.orderItems.forEach(a=>{
        a.orderFromUser.forEach( b =Z {})
      })
    }
