<template>
  <div class="cart-page">
    <div class="container py-4">
      <div class="line-bottom heading-text mb-4">
        <h1 class="main_title"><span>Cart</span></h1>
      </div>
      <div class="row">
        <div class="col-12 col-sm-12 col-md-9">
          <div class="cart-contain">
            <div class="row header-cart justify-content-between text-center">
              <div class="col-12">Number</div>
              <div class="col-12 d-flex justify-content-start">Product</div>
              <div class="col-12 d-flex justify-content-end">Category</div>
              <div class="col-12">Price</div>
         
            </div>
            <div class="row align-items-center text-center body-cart">
              <div class="col-12" v-if="!cartObjects.length">
                This cart is empty! In order to order you need to fill it up!
              </div>
             
                <CartItem
                  v-for="item in cartObjects"
                  :key="item.id"
                  :item="item"
                  class="col-12 col-sm-12 col-md-12 item-cart"
                />
              
            </div>

            <div class="routing-back mt-5">
              <div class="row">
                <div class="col-12">
                  <router-link to="/" class="back-nav">
                    Go back to products !
                  </router-link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="col-12 col-sm-12 col-md-3">
          <div class="cart-contain">
            <h1>Overview</h1>
            <CartTotal v-if="cartObjects.length > 0" />
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import CartItem from "@/components/Cart/CartItem.vue";
import CartTotal from "@/components/Cart/CartTotal.vue";
import { mapGetters } from "vuex";
export default {
  name: "Cart",

  components: {
    CartItem,
    CartTotal,
  },
  computed: {
    ...mapGetters(["cartObjects"]),
  },
};
</script>

<style lang="scss" scoped></style>
