    ...
    <div class="row ">
      <div class="col-xxl-2 col-xl-3 col-lg-3 mb-15">
        <div class="row flex-column mt-4">
         ...
         <b-button variant="info" @click='sortBy3("title")' class="mt-3">Z-A</b-button>
        </div>
...
export default {
  ...
  methods:{ 
     ...
     sortBy2(prop){
      this.$store.state.games.sort((a, b) => b[prop] - a[prop] ? -1 : 1);
    },
     sortBy3(prop){
      this.$store.state.games.sort((a, b) => b[prop] - a[prop] ? -1 : 1).reverse();
    }
  },
  computed: {
    ...mapState(["games"]),
